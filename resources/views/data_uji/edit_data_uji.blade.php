...
<body>
  ...
  <main id="main" class="main">
    <section class="section dashboard">
        <div class="card">
            <div class="card-body">
            <h5 class="card-title">Edit Data Uji</h5>
            <form action="{{ url('perubahan-data-uji/' .$varuji->id) }}" method="post" class="row g-3">
                {{ csrf_field() }}
                <div class="col-12">
                    <label class="form-label">Nama Petani</label>
                    <select name="nik_petani" class="form-control form-control-user" disabled>
                        <option value="" disabled selected>{{ $varuji -> petani -> nama }}</option>
